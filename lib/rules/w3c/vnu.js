...
VNU.parseRule = function(payload, line_strs, rule, fn) {
  ...
  VNU.cleanKnown(rule.message || '', function(err, message){
    ...
    // debug
    payload.debug('parseRule', '-> (default) ' + rule.message)

    // remove breaks
    var cleanedMessage = rule.message.split('\n').join('');

    // loop and replace all the idenfiers
    while(match = re.exec(cleanedMessage)) {
      ...
      // skip if known
      if(VNU.knownElements.indexOf(cleaned_identifier.toLowerCase()) !== -1) {
    ...
    // debug
    payload.debug('parseRule', '-> (parsed)' + message)
    payload.debug('parseRule', '* ' + identifiers)
